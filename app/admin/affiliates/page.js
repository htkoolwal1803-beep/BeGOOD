      ...
      <div className="brand-page min-h-screen flex items-center justify-center p-4">
        <div className="brand-panel p-8 max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="text-[#59615b] mt-2">Enter password to manage affiliates</p>
          </div>
      ...
      <div className="brand-page min-h-screen p-6">
        <div className="max-w-6xl mx-auto">
          <button
            ...
            className="flex items-center text-[#6f8a74] hover:underline mb-6"
          >
          ...
          <div className="brand-card p-6 mb-6">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-[#6b736d]">Code: {selectedAffiliate.affiliate.code}</p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-[#f4ecdd] rounded-lg p-4">
                <p className="text-[#59615b] text-sm">Total Orders</p>
                <p className="text-2xl font-bold text-[#6f8a74]">{selectedAffiliate.affiliate.stats.totalOrders}</p>
              </div>
              <div className="bg-[#f4ecdd] rounded-lg p-4">
                <p className="text-[#59615b] text-sm">Total Units Sold</p>
                <p className="text-2xl font-bold text-[#6f8a74]">{selectedAffiliate.affiliate.stats.totalUnits}</p>
              </div>
              <div className="bg-[#f4ecdd] rounded-lg p-4">
                <p className="text-[#59615b] text-sm">Total Revenue</p>
                <p className="text-2xl font-bold text-[#6f8a74]">₹{selectedAffiliate.affiliate.stats.totalRevenue}</p>
              </div>
              <div className="bg-[#f4ecdd] rounded-lg p-4">
                <p className="text-[#59615b] text-sm">Link Clicks</p>
                <p className="text-2xl font-bold text-[#6f8a74]">{selectedAffiliate.affiliate.stats.clicks}</p>
              </div>
            </div>

            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-[#6b736d]">Phone</p>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {selectedAffiliate.orders.length === 0 ? (
                    <tr>
                      <td colSpan="7" className="p-8 text-center text-[#6b736d]">
                    ...
                    selectedAffiliate.orders.map((order) => (
                      <tr key={order.orderId} className="border-b hover:bg-[#eef3ea]">
                        ...
                        <td className="p-4">
                          ...
                          <p className="text-sm text-[#6b736d]">{order.phone}</p>
                        </td>
                        <td className="p-4">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[#59615b]">Manage your affiliate partners and track their performance</p>
          </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6f8a74]"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="brand-card p-4">
            <p className="text-[#59615b] text-sm">Total Affiliates</p>
            <p className="text-2xl font-bold text-[#6f8a74]">{affiliates.length}</p>
          </div>
          <div className="brand-card p-4">
            <p className="text-[#59615b] text-sm">Active Affiliates</p>
            <p className="text-2xl font-bold text-green-600">{affiliates.filter(a => a.isActive).length}</p>
          </div>
          <div className="brand-card p-4">
            <p className="text-[#59615b] text-sm">Total Orders via Affiliates</p>
            <p className="text-2xl font-bold text-[#6f8a74]">
              {affiliates.reduce((sum, a) => sum + (a.stats?.totalOrders || 0), 0)}
            </p>
          </div>
          <div className="brand-card p-4">
            <p className="text-[#59615b] text-sm">Total Revenue via Affiliates</p>
            <p className="text-2xl font-bold text-[#6f8a74]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {affiliates.length === 0 ? (
                  <tr>
                    <td colSpan="8" className="p-8 text-center text-[#6b736d]">
                      No affiliates yet. Click "Add Affiliate" to create one.
                    </td>
                  </tr>
                ) : (
                  affiliates.map((affiliate) => (
                    <tr key={affiliate.code} className="border-b hover:bg-[#eef3ea]">
                      <td className="p-4">
                        <div>
                          <p className="font-medium">{affiliate.name}</p>
                          <p className="text-sm text-[#6b736d]">{affiliate.phone || affiliate.email || '-'}</p>
                        </div>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-[#6f8a74]">{affiliate.code}</span>
                          <button
                            onClick={() => copyAffiliateLink(affiliate.code)}
                            className="text-[#8b938b] hover:text-[#6f8a74]"
                      ...
                      <td className="p-4">
                        <button
                          ...
                          className="text-[#6f8a74] hover:underline flex items-center"
                        >

...
export async function DELETE(request) {
  ...
  let coupon = null
  if (body.couponCode) {
    coupon = await db.collection('coupons').findOne({ code: String(body.couponCode).toUpperCase(), isActive: true })
  ...
  couponDiscount = Math.max(0, Math.min(couponDiscount, subtotal + shippingFee))
  return { products, subtotal, shippingFee, couponDiscount, totalAmount: subtotal + shippingFee - couponDiscount, coupon }
}

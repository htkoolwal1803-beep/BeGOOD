...
async function metaGet(path, token, fields) {
  const params = new URLSearchParams({ fields, access_token: token })
  return responseJson(
    await fetch(`https://graph.facebook.com/${META_VERSION}/${path}?${params}`, {
      cache: 'no-store'
    }),
    'Instagram API request failed'
  )
...
async function waitForInstagramContainer(containerId, token, label) {
  for (let attempt = 0; attempt < 15; attempt += 1) {
    const container = await metaGet(containerId, token, 'status_code,status')
    const statusCode = String(container.status_code || '').toUpperCase()
    if (statusCode === 'FINISHED') return container
    if (statusCode === 'ERROR' || statusCode === 'EXPIRED') {
      throw new Error(`${label} failed while Instagram processed it: ${container.status || statusCode}`)
    }
    if (attempt < 14) await new Promise((resolve) => setTimeout(resolve, 1000))
  }
  throw new Error(`${label} was not ready after 15 seconds. Retry this platform once in Social Admin.`)
}
...
async function publishInstagram(post) {
  ...
  await Promise.all(childIds.map((id, index) => waitForInstagramContainer(id, token, `Instagram carousel image ${index + 1}`)))
  ...
  await waitForInstagramContainer(carousel.id, token, 'Instagram carousel')

...
function enabledPlatformSet(input) {
  if (input == null) return new Set(['linkedin', 'instagram'])
  if (!Array.isArray(input)) throw new Error('Enabled platforms must be a list')
  const enabled = new Set(input.map((item) => String(item).toLowerCase()))
  for (const platform of enabled) {
    if (!['linkedin', 'instagram'].includes(platform)) throw new Error('Invalid enabled platform')
  }
  if (!enabled.size) throw new Error('Enable at least one platform')
  return enabled
}
...
export async function createSocialPost(input) {
  ...
  const enabled = enabledPlatformSet(input.enabledPlatforms)
  const instagramImages = Array.isArray(input.instagramImages) ? input.instagramImages : []
  if (enabled.has('instagram') && (instagramImages.length < 2 || instagramImages.length > 10)) {
  ...
  const linkedin = enabled.has('linkedin')
    ? {
        enabled: true,
        ...
        document: {
          url: cleanUrl(input.linkedinDocumentUrl, 'LinkedIn document'),
          title: documentTitle,
  ...
  const instagram = enabled.has('instagram')
    ? {
        enabled: true,
        status: 'waiting_approval',
...
export async function approvePost({ postId, userId, eventTs }) {
  ...
  for (const platform of ['linkedin', 'instagram']) {
    ...
    if (!current || current.status === 'disabled') continue
...
export async function sendDueApprovalRequests(now = new Date()) {
  ...
  if (!posts.length && isAlternateSocialDate(date)) {
    const existing = await db.collection('social_posts').countDocuments({ scheduledDate: date }, { limit: 1 })
    if (!existing) {
